    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.token) {
          ...
          const checkoutUrl = `https://checkout.tankstopfischer.de/checkout?token=${data.token}`;
      ...
      toast({
        ...
        description: "Bitte versuchen Sie es später erneut oder kontaktieren Sie uns per E-Mail an info@tankstopfischer.de.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
